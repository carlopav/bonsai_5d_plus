...
- **Fixed** — un prezzo piatto, con lista **editabile inline** (categoria, nome, valore per riga) e un'**unica unità di misura condivisa** per l'intera voce, scelta da un elenco di unità comuni (mq, mc, m, kg, t, h, cad) con `Custom…` per testo libero. *(La scelta dell'unità basata sulle unità effettive del progetto è in fase di progettazione — vedi nota più sotto.)*
...
- **Cost Item Editor** — three-panel editor for a single `IfcCostItem`, loaded manually or automatically (auto-load toggle follows Bonsai's own active cost item):
  ...
  - *Cost Values*: three mutually-exclusive price modes, each with an explicit **gate** — if the item's current IFC state doesn't match the selected mode, only a "Clear cost values and…" button is shown, so the active mode is never ambiguous.
    ...
    - *Fixed*: a flat price with an **inline-editable list** (category, name, value per row) and **one shared unit of measure** for the whole item, picked from a list of common units (mq, mc, m, kg, t, h, cad) with `Custom…` for free text. *(Picking from the project's actual declared units is being designed — see note below.)*
