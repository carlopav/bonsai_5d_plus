...
**Opzione di export `Round Values Before Summing`** (finestra di dialogo di BoQ PDF, ODS e Quadro Incidenza Manodopera, **attiva di default**): spenta, ripristina l'aritmetica grezza a piena precisione — le stesse cifre del pannello Cost nativo di Bonsai — utile per confrontare quando un totale va spiegato. Nel PDF i subtotali di sezione tornano allora al `TotalPrice` di ifc5d; nell'ODS le formule vive passano da `=ROUND(qta*prezzo;2)` a `=qta*prezzo`.

**Coerenza tra documenti.** Il **Quadro di Incidenza Manodopera** riusa le stesse funzioni di somma del BoQ per la colonna del costo totale, così ogni importo — voce, sezione, totale generale — coincide con il BoQ della stessa schedule, in entrambe le modalità. L'**analisi prezzi** riporta i valori **letti dall'IFC**, non ricalcolati: gli importi dei componenti, i ricarichi (sicurezza, spese generali, utile) e l'arrotondamento sono gli `AppliedValue` stotti; la percentuale, letta dal `Name` del CV, serve solo da etichetta ("14,0% di …"). Così il PREZZO FINALE della scheda è la somma dei componenti stotti — lo stesso valore che ifc5d ricava per il prezzo della voce nel BoQ — e i due documenti coincidono anche se l'analisi è stata modificata dopo l'applicazione (es. un componente editato nell'editor nativo di Bonsai). L'unico valore derivato resta il P.U. (`AppliedValue / quantità`), nella tolleranza sull'eval. Il CV sommario porta anche un totale in cache nel suo `AppliedValue`: la scheda lo legge solo come **controllo di coerenza** — se non coincide con la somma dei componenti, l'analisi è stata toccata dopo l'applicazione e la scheda stampa un avviso in rosso, continuando a riportare come prezzo la somma dei componenti.
...
**Export option `Round Values Before Summing`** (BoQ PDF, ODS and Labor Cost Breakdown dialog, **on by default**): turn it off to restore the raw full-precision arithmetic — the same figures Bonsai's own Cost panel shows — for comparison when a total needs explaining. In the PDF, section subtotals then fall back to ifc5d's `TotalPrice`; in the ODS the live formulas switch from `=ROUND(qty*rate;2)` to `=qty*rate`.

**Cross-document consistency.** The **Labor Cost Breakdown** reuses the BoQ's own summing functions for its total-cost column, so every amount — item, section, grand total — matches the BoQ for the same schedule, in both modes. The **rate analysis** reports the values **read from IFC**, not recomputed: the component amounts, the markups (safety, overhead, profit) and the rounding are the stored `AppliedValue`s; the percentage, read from the CV `Name`, serves only as a label ("14.0% di …"). So the sheet's final price is the sum of the stored components — the same value ifc5d derives for the item's price in the BoQ — and the two documents agree even if the analysis was edited after it was applied (e.g. a component changed in Bonsai's native editor). The only derived value left is the unit price (`AppliedValue / quantity`), within the eval tolerance. The summary CV also caches a total in its `AppliedValue`; the sheet reads it only as a **consistency check** — if it disagrees with the sum of the components, the analysis was touched after it was applied, and the sheet prints a red warning while still reporting the component sum as the price.
